...
def align_prev_to_gen(
    *,
    ...
    #     gen_batch.non_tensor_batch["prompt"][::n_repeat],  # take first row of each block
    #     skip_special_tokens=True,
    # )
    ref_prompts = gen_batch.non_tensor_batch["prompt"][::n_repeat]
    ...
    for txt in ref_prompts:
        if len(bucket[txt]) < n_repeat:
            raise ValueError(f"Prompt «{txt[:60]}…» appears fewer than {n_repeat} times in prev_data – cannot align.")
